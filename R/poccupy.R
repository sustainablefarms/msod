...
#' If `LVvals` is an array with 3 dimensions, then the third dimension corresponds to the posterior distribution draw (same as occ.b_arr).
#' In this situation the array must have only one row: the row corresponding the model site.
#' @param ldet.b_arr A 3-array of LV loadings. If `NULL` then model is assumed to not have latent variables.
...
poccupy.ModelSite <- function(Xocc, occ.b_arr, ldet.b_arr = NULL, LVvals = NULL){
  if (is.null(ldet.b_arr) && is.null(LVvals)){ #model doesn't have latent variables
  ...
  if (length(dim(LVvals)) == 3){ #model has LV and the probabilities are conditional on the fitted LV value.
    ...
    pocc_l <- lapply(1:dim(LVvals)[[3]],
           function(drawid){
             pocc <- poccupy.ModelSite.theta(Xocc, 
                                     drop_to_matrix(occ.b_arr[,, drawid, drop = FALSE]),
                                     drop_to_matrix(ldet.b_arr[,, drawid, drop = FALSE]),
                                     drop_to_matrix(LVvals[,, drawid, drop = FALSE]))
             return(pocc)
           })
  }
  
  if (!is.null(ldet.b_arr) && length(dim(LVvals)) == 2){ #this is when LVvals are meant to be marginalised for each theta
    pocc_l <- lapply(1:dim(occ.b_arr)[[3]],
                     function(drawid){
                       pocc <- poccupy.ModelSite.theta(Xocc, 
                                                       drop_to_matrix(occ.b_arr[,, drawid, drop = FALSE]),
