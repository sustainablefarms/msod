...
poccupy.ModelSite <- function(Xocc, occ.b_arr, lv.b_arr = NULL, LVvals = NULL){
  if (is.null(lv.b_arr) && is.null(LVvals)){ #model doesn't have latent variables
  ...
  if (!is.null(lv.b_arr) && length(dim(LVvals)) == 2){ #this is when LVvals are meant to be marginalised for each theta
    pocc_l <- lapply(1:dim(occ.b_arr)[[3]],
                     function(drawid){
                       pocc <- poccupy.ModelSite.theta(Xocc, 
                                                       drop_to_matrix(occ.b_arr[,, drawid, drop = FALSE]),
