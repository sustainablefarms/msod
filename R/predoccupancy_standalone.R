...
#' The chance of occupancy of each species (the model without latent variables assumes species are independent) is then computed using the supplied parameter matrix occ.b
#' @examples
...
#' poccupancy <- poccupancy_standalone_nolv(Xocc, XoccProcess, occ.b)
...
#' @export
poccupancy_standalone_nolv <- function(Xocc, XoccProcess, occ.b){
  XoccStd <- apply.designmatprocess(XoccProcess, Xocc)

  # occupancy probability
  ModelSite.Occ.eta <- as.matrix(XoccStd) %*% t(occ.b)
...
#' @export
multisiterichness_nolv <- function(Xocc, XoccProcess, occ.b){
  poccupancy <- poccupancy_standalone_nolv(Xocc, XoccProcess, occ.b)
