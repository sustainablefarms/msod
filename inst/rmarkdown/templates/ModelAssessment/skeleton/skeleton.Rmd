...
filenames <- lapply(modelspecs, function(x) gsub("./tmpdata/", "./fittedmodels/", x$filename))
...
inputdata <- readRDS("~/dlinkingdatacode/private/data/clean/7_2_10_input_data.rds")
...
*Comment on effective sample size. Should be similar order of magnitude to the number of MCMC samples.*
...
gwk %>%
  ...
  theme(strip.text.x.top = element_text(angle = 90)) +
  ggtitle("Geweke Diagnostic Statistics")
```

Geweke statistic values between -2 and 2 are good.

#### Less Standard Assessment
Below visualisations of Geweke values is not standard. Relies of Geweke values being independen, which is not true in general.
...
elpd_compare(elpds = do.call(cbind, lapply(waics_l, function(x) x$waic$pointwise[, 1])))
elpd_compare(elpds = do.call(cbind, lapply(waics_l, function(x) x$loo$pointwise[, 1])))
...
```{r EnumspeciesInsample, fig.height = 9}
