...
cleanersummaries %>%
  ...
  ggplot() +
  facet_grid(rows = vars(type), cols = vars(Model), scales = "free") +
  geom_point(aes(y = bugsvarname, x = SSeff) ) +
  geom_vline(xintercept = fittedmods[[1]]$sample * length(fittedmods[[1]]$mcmc) / 4, col = "blue", lty = "dashed") +
  scale_y_discrete(name = "Variable Name") +
  theme(strip.text.x.top = element_text(angle = 90)) +
...
```{r biodiversity_ci_coverage}
Enum_coverage(obsnumbers, Enum_det, Vnum_det)[["mean"]] %>%
  tibble::enframe(name = "Model", value = "Pcnt. Coverage") %>%
  ggplot() +
  geom_point(aes(x = `Pcnt. Coverage`, y = Model)) +
  geom_vline(xintercept = 0.95, col = "blue") +
  ggtitle("Coverage of approx. 95% credence intervals for biodiversity")
```
