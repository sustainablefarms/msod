...
test_that("Expected Biodiversity is Historically Consistent", {
  Enspecies_condLV <- predsumspecies(fit, UseFittedLV = TRUE)
  expect_known_output(Enspecies_condLV, file = "Especrich_condLV.txt", print = TRUE, update = TRUE)
  
  set.seed(1341) #for simulated LV values
  Enspecies_margLV <- predsumspecies(fit, UseFittedLV = FALSE)
  expect_known_output(Enspecies_margLV, file = "Especrich_margLV.txt", print = TRUE, update = TRUE)
})

test_that("Expected Individual Species Detections are Historically Consistent", {
  En_condLV <- Endetect_modelsite(fit, type = 1, conditionalLV = TRUE)
  expect_known_output(En_condLV, file = "En_condLV.txt", print = TRUE, update = TRUE)
  
  En_margLV <- Endetect_modelsite(fit, type = 1, conditionalLV = FALSE)
  expect_known_output(En_margLV, file = "En_margLV.txt", print = TRUE, update = TRUE)
})
